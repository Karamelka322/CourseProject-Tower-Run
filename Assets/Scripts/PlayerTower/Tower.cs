...
public class Tower : MonoBehaviour
{
    ...
    public int TowerSize => _humanInTower.Count;
    ...
    private void SpawnHumans(int humanCount)
    {
        ...
        {
            Human sawnedHuman = _humansTemplate[Random.Range(0, _humansTemplate.Length)];
            _humanInTower.Add(Instantiate(sawnedHuman, spawnPoint, Quaternion.identity, transform));

            _humanInTower[i].transform.localPosition = new Vector3(0, _humanInTower[i].transform.localPosition.y, 0);
            spawnPoint += sawnedHuman.FixationPoint.position;
        }
    }
    ...
    private float CheckDistanceY(Transform distanceChecker, Transform fixationPoint)
    {
        ...
        Vector3 humanFixationPointY = new Vector3(0, fixationPoint.position.y, 0);
